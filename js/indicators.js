...
const Indicators = (() => {
  ...
  function calculateMCPredict(ohlcv) {
    ...
    // 5. ML (kNN) simplified logic 
    const knnLookback = 100;
    const knnK = 5;
    const mlScore = new Array(c.length).fill(0);

    for(let i = knnLookback + 10; i < c.length; i++) {
      const f1Now = rsiVal[i] / 100.0;
      const f2Now = macdRes.histogram[i] / (atrVal[i] + 0.0001);
      const f3Now = (c[i] - bbRes.lower[i]) / (bbRes.upper[i] - bbRes.lower[i]);

      const distances = [];
      for(let j = i - knnLookback; j <= i - 10; j++) {
        const f1Past = (rsiVal[j] || 50) / 100.0;
        const f2Past = (macdRes.histogram[j] || 0) / (atrVal[j] + 0.0001);
        const f3Past = (c[j] - bbRes.lower[j]) / (bbRes.upper[j] - bbRes.lower[j]);

        const dist = Math.sqrt(Math.pow(f1Now - f1Past, 2) + Math.pow(f2Now - f2Past, 2) + Math.pow(f3Now - f3Past, 2));
        const futureRet = c[j] - c[j - 5]; 
        const vote = futureRet > 0 ? 1 : -1;
        distances.push({ dist, vote });
      }

      distances.sort((a,b) => a.dist - b.dist);
      let sumVotes = 0;
      for(let k=0; k<Math.min(knnK, distances.length); k++) {
        sumVotes += distances[k].vote;
      }
      mlScore[i] = (sumVotes / knnK) * 100;
    }
