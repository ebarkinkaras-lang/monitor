...
function triggerKernelPanic(msg) {
  let errBox = document.getElementById('sys-error-box');
  if(!errBox) {
    errBox = document.createElement('div');
    errBox.id = 'sys-error-box';
    errBox.style.cssText = "position:fixed; top:40px; right:20px; background:#1A0000; color:#FF1744; padding:12px; z-index:9999; font-family:monospace; font-weight:bold; box-shadow:0 0 20px #FF1744; border: 2px solid #FF1744; text-shadow: 0 0 5px #FF1744;";
    document.body.appendChild(errBox);
  }
  errBox.innerHTML = `[SYS_ERR] KERNEL PANIC<br/>>> ${escapeHTML(msg)}<br/>>> YEREL ÖNBELLEK DEVREDE.`;
...
async function fetchWithProxy(url) {
  for (let i = 0; i < CORS_PROXIES.length; i++) {
    ...
    try {
      ...
      console.warn(`Proxy ${proxyIndex} failed`);
    }
  }
  triggerKernelPanic(`BAĞLANTI REDDEDİLDİ: ${url.substring(0, 40)}...`);
