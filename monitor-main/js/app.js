...
const App = (() => {
  ...
  async function boot() {
    ...
    setTimeout(() => CalendarModule.init(), 1700);
  }
...
const CalendarModule = (() => {
  const MOCK_EVENTS = [
    { time: "14:00", country: "TR", event: "TCMB Faiz Kararı", impact: "high", exp: "%45.0" },
    { time: "15:30", country: "US", event: "Tarım Dışı İstihdam", impact: "high", exp: "200K" },
    { time: "10:00", country: "TR", event: "TÜFE (Yıllık)", impact: "high", exp: "%69.1" },
    { time: "11:00", country: "EU", event: "Euro Bölgesi PMI", impact: "medium", exp: "48.5" }
  ];
  ...
  function init() {
    ...
    let html = '';
    MOCK_EVENTS.forEach(item => {
      const color = item.impact === 'high' ? 'var(--red)' : '#FFB000';
      html += `
        <div class="news-item" style="border-left-color: ${color}; padding-left:8px; margin-bottom:6px;">
          <div style="display:flex; justify-content:space-between; color:var(--amber);">
            <span style="font-weight:bold;">[${item.country}] ${item.event}</span>
            <span style="color:var(--amber-dim);">${item.time}</span>
          </div>
          <div style="font-size:13px; color:#00E676; margin-top:2px;">
            Beklenti: ${item.exp}
          </div>
        </div>`;
    });
